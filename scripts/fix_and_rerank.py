            ...
            try:
                ...
                for edu in edu_data:
                    ...
                    if "phd" in degree_str or "doctor" in degree_str:
                        ...
                        "mca" in degree_str
                        or "msc" in degree_str
                        or "m.tech" in degree_str
                        or "master" in degree_str
                    ):
                        level = EducationLevel.MASTERS
                    elif "mba" in degree_str:
                        level = EducationLevel.MBA
                    elif (
                        "b.tech" in degree_str
                        or "b.sc" in degree_str
                        or "be" in degree_str
                        or "btech" in degree_str
                        or "bachelor" in degree_str
                    ):
...
for feat_row in feature_rows:
    ...
    evidence_count = (
        jd_overlap
        + (1 if yoe >= 3 else 0)
        + (1 if edu_score > 0.5 else 0)
        + (1 if len(profile.work_experience) > 1 else 0)
    )
    if evidence_count >= 4:
...
for r in top100[:10]:
    print(
        f"    #{r.rank} {r.candidate_id} | Score: {r.overall_score:.4f} | {r.hiring_recommendation.value} | {r.confidence_level.value}"
    )
