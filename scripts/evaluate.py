...
def run(
    ...
    perturbation: float = typer.Option(
        0.05, "--noise", help="Feature noise level for stability test"
    ),
) -> None:
    """
    ...
    for _ in range(n_trials):
        ...
        for c in candidates:
            pc = {
                k: (
                    min(1.0, max(0.0, v + random.gauss(0, perturbation)))
                    if k != "candidate_id"
                    else v
                )
                for k, v in c.items()
    ...
    stability_scores = [len(base_top_k & trial_set) / top_k for trial_set in top_k_sets]
    avg_stability = np.mean(stability_scores)
    min_stability = np.min(stability_scores)

    stab_table = Table(
        title=f"2. Ranking Stability (noise={perturbation}, {n_trials} trials)", border_style="blue"
    )
    ...
    abl_table = Table(
        title="3. Feature Ablation Analysis (impact on mean rank shift)", border_style="blue"
    )
    ...
    conf_table = Table(
        title="4. Confidence Distribution (profile completeness proxy)", border_style="blue"
    )
    ...
    console.print(
        f"  Score IQR            : {np.percentile(scores, 75) - np.percentile(scores, 25):.4f} "
        f"({'good spread' if np.percentile(scores, 75) - np.percentile(scores, 25) > 0.10 else 'concentrated'})"
    )
    console.print(
        f"  Top-{top_k} Stability  : {avg_stability:.1%} mean overlap under {perturbation} noise"
    )
